...
import { parseJSONFromResponse, validateWorldSetting, validateNPCSetting, validateSceneSetting } from './utils.js';
...
/**
 * Process document and extract visual game data using Claude
 */
export async function processVisualDocument(fileId, filePath, fileType) {
  console.log(`📄 Processing visual game document: ${fileId}`);
  ...
  try {
    ...
    // STEP 1/3: Generate worldSetting
    console.log('🤖 Step 1/3: Generating worldSetting...');

    const worldSettingPrompt = `你是一个专业的视觉小说游戏内容分析AI。请分析文档并生成世界设定JSON。

## worldSetting.json (世界设定)
**必需字段**:
- title (string): 游戏标题
- background (string): 世界背景故事（详细）
- preamble (string): 游戏前言/开场白
...
**可选字段**:
- summary (string): 简短摘要
- Theme (array): 主题数组（例如：["冒险", "魔幻"]）
- WorldRules (object): 世界规则，键值对形式
- player.age (number): 年龄
- player.personality (string): 性格
- player.tone (string): 语气

## 分析要求
1. 仔细阅读文档，识别故事背景、主角等信息
...
${documentText.length > maxChars ? '\n（注：文档过长，已截取前50000字符）' : ''}`;
    ...
    // STEP 2/3: Generate npcSetting
    console.log('🤖 Step 2/3: Generating npcSetting...');

    const npcSettingPrompt = `你是一个专业的视觉小说游戏内容分析AI。请根据文档和已生成的世界设定，生成NPC设定JSON。
...
主角: ${worldSetting.player.name}`;
    ...
    // STEP 3/3: Generate sceneSetting
    console.log('🤖 Step 3/3: Generating sceneSetting...');

    const sceneSettingPrompt = `你是一个专业的视觉小说游戏内容分析AI。请根据文档、世界设定和NPC设定，生成场景设定JSON。
...
- items (array): 可用物品数组
- events (array): 可能发生的事件数组

## 分析要求
1. 仔细阅读文档，识别故事中的场景和地点
2. 为场景生成合适的英文ID（小写，下划线分隔，有意义）
3. 确保所有必需字段都有值
4. 尽可能填充可选字段以丰富游戏内容
5. 至少生成3个场景
...
## 已生成的NPC列表：
${npcSetting.npcs.map(npc => `- ${npc.name} (${npc.id})`).join('\n')}`;
    ...
    const fileDir = path.join(VISUAL_TEMP_DIR, fileId);
    ...
    console.log(`✅ Visual game data saved to: ${fileDir}`);
    ...
    console.error('Error processing visual game document:', error);
    throw error;
  }
}
 ...
 */
export function getVisualGameFiles(fileId) {
  try {
    const fileDir = path.join(VISUAL_TEMP_DIR, fileId);
...
/**
 * List all available visual game files
 */
export function listVisualGameFiles() {
  try {
    if (!fs.existsSync(VISUAL_TEMP_DIR)) {
      return [];
    }

    const files = fs.readdirSync(VISUAL_TEMP_DIR);

    const fileList = files.map(fileId => {
      try {
        const metadataPath = path.join(VISUAL_TEMP_DIR, fileId, 'metadata.json');
        if (fs.existsSync(metadataPath)) {
...
/**
 * Delete visual game files
 */
export function deleteVisualGameFiles(fileId) {
  try {
    const fileDir = path.join(VISUAL_TEMP_DIR, fileId);

    if (fs.existsSync(fileDir)) {
      fs.rmSync(fileDir, { recursive: true, force: true });
      console.log(`🗑️ Deleted visual game files: ${fileId}`);
      return true;
    }
